        ...
        <div className="flex justify-between items-center">
          ...
          <div className="relative py-2 group">
            ...
            <div className="absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#282142] border-gray-600 text-gray-100 hidden group-hover:block">
                ...
                <p onClick={()=>navigate('/profile')}className='cursor-pointer text-sm'>EditProfile</p>
      ...
      <div className="flex flex-col">
        {filterdUsers.map((user,index)=>(
            ...
            key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id===user._id && 'bg-[#282142]/50'}`}>
              ...
              {unseenMessages[user._id]>0 && <p className=" absolute top-4 right-4 text-xs h-5 w-5 flex justify-center items-center rounded-full bg-purple-400/50">{unseenMessages[user._id]}</p>}
            </div>
        ))}
